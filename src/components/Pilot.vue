...
export default {
  ...
  computed: {
    ...
    pilotCode() {
      ...
      let identName = ''
      identLastNameParts.forEach((part: any) => {
        identName += `${part}.`
      })
    ...
    pilotInfo() {
      ...
      function resolveGear(type: string, item: any, idx: number, arr: Array<any>) {
        if (item && Object.prototype.hasOwnProperty.call(item, 'flavorName') && typeof item.flavorName !== 'undefined' && !(item.flavorName.length > 0)) {
          item.flavorName = knownGear.find((obj: any) => { return item.id === obj.id }).name
        }
        else {
      ...
      pilotInfo.loadout.armor.forEach((item: any, index: number, array: Array<any>) => {
        resolveGear('armor', item, index, array)
      })
      pilotInfo.loadout.weapons.forEach((item: any, index: number, array: Array<any>) => {
        resolveGear('weapon', item, index, array)
      })
      pilotInfo.loadout.gear.forEach((item: any, index: number, array: Array<any>) => {
  ...
  methods: {
    ...
    getActiveMech() {
      ...
      if (mech) {
        ...
        this.pilot.mechs[0] ? this.activeMech = this.pilot.mechs[0] : lancerData.frames.find((obj: any) => { return obj.id === 'missing_frame' })
      }

      const knownFrames = [...lancerData.frames, ...ktbData.frames, ...nrfawData.frames, ...longrimData.frames]
      let frame = knownFrames.find((obj: any) => {
